...
export default function HomePage() {
  return <VvaiProductShell>
    ...
    <HomeSection className="pt-24 md:pt-32">
      <div className="flex flex-wrap items-end justify-between gap-8">
        ...
        <Button variant="outline" asChild><Link href="/products">View Products <ArrowRight size={15} /></Link></Button>
      </div>
    ...
    <HomeSection className="bg-[linear-gradient(180deg,#030303_0%,#041012_50%,#030303_100%)]">
      ...
      <h2 className="mt-6 max-w-3xl text-3xl font-semibold tracking-tight md:text-5xl">Practical systems before oversized software.</h2>
      <div className="mt-10 grid gap-5 md:mt-12 md:grid-cols-3">
        {selectedWork.map(([title, text, Icon]) => <Card key={title} className="border-white/[.10] bg-black/25 p-6">
    ...
    <HomeSection>
      <div className="grid gap-12 lg:grid-cols-[.9fr_1.1fr] lg:items-start">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {audiences.map(([title, text]) => <Card key={title} className="border-white/[.10] bg-black/25 p-6">
    ...
    <HomeSection className="pb-24 md:pb-32">
      ...
      <div className="mt-6 grid gap-10 lg:grid-cols-[1fr_1.2fr] lg:items-start">
        <div>
          ...
          <Button className="mt-8" asChild><Link href="/contact">Discuss a Project <ArrowRight size={15} /></Link></Button>
        </div>
        <div className="grid gap-5">
          {mvpSystems.map(([title, text, Icon]) => <Card key={title} className="flex gap-5 border-white/[.10] bg-black/25 p-6">
